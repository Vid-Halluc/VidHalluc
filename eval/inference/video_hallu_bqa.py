...
if __name__ == "__main__":
    ...
    results = {}
    for index, question_list in videos_dict.items():
        video_id = list(question_list[0]['a'].keys())[0].rsplit('_', 2)[0]
        if index in results:
            continue
        temp = []
        for question in question_list:
            temp_inner = {}
            temp_inner['q'] = question['q']
            temp_inner['a'] = {}
            for clip in question['a']:
                args.prompt = "{desc} Only answer with a single word 'Yes' or 'No'.".format(desc=question['q'])
                args.video_path = video_folder + clip + '.mp4'
                print(args.video_path)
                temp_inner['a'][clip] = run_inference(args, cfg_pretrained, tokenizer, model, image_processor, context_len)
            temp.append(temp_inner)
        results[index] = temp
        with open('LLaVA-Video_Hallu_bqa.json', 'w') as f:
